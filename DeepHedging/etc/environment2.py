...
class TradeEnv(py_environment.PyEnvironment):
    ...
    def __init__(self, df, balance):
        ...
        self._action_spec = array_spec.BoundedArraySpec(shape=(), dtype=np.float64, minimum=0.0, maximum=2.0, name='action')

        # Observations : [stock 주가 30일치]
        self._observation_spec = array_spec.BoundedArraySpec(shape=(3,), dtype=np.float64, minimum=0,
                                                             name='observation')
    ...
    def _reset(self):  # 필수 return 타입 : ts.TimeStep
        ...
        return ts.restart(np.array([self.df[self.episode_step], self.getTotalValue(), self.reward], dtype=np.float64))  # 초기화
    ...
    def _step(self, action):  # 필수 return 타입 : ts.TimeStep
        ...
        if self.episode_step >= 30:  # episode 종료 조건 (만기일 도달)
            ...
            return ts.termination(np.array([self.df[self.episode_step], self.getTotalValue(), self.reward], dtype=np.float64),
                                  self.reward + self.start_balance * 0.2)
        else:
            self.episode_step += 1
            return ts.transition(np.array([self.df[self.episode_step], self.getTotalValue(), self.reward], dtype=np.float64),
                                 self.reward, discount=1.0)
    ...
    def action(self, action):
        print(self.episode_step, self.getTotalValue(), self.reward)
