...
from ac2_model import a2c_Model
...
class A2CAgent:
    ...
    def train(self, env, batch_size=64, updates=250):
        ...
        for update in range(updates):
            ...
            _, next_value = self.model.action_value(
                next_obs[None, :])  # finding the estimated value function of the last observation
            returns, advs = self.return_advantages(rewards, dones, values,
                                                   next_value)  # finding the estimated returns and the advantages
            acts_and_advs = np.concatenate([actions[:, None], advs[:, None]],
                                           axis=-1)  # passing actions and advantages together for the loss function
    ...
    def test_model(self, env, network, episodes):
        self.model.network = load_model(network, custom_objects={'logits_loss': self.logits_loss,
                                                                 'valuefn_loss': self.valuefn_loss})
        episode_rewards = []
